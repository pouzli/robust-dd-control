...
def test_jacobian_near_zero_for_polynomial_basis() -> None:
    # Use richer sampling so LS consistently recovers local linear terms.
    x1 = np.linspace(-0.5, 0.5, 15)
    x2 = np.linspace(-0.5, 0.5, 15)
    grid = np.array([(a, b) for a in x1 for b in x2], dtype=float)

    xdot = np.column_stack(
        (
            -grid[:, 0] + grid[:, 1] + 0.8 * grid[:, 0] * grid[:, 1],
            -2.0 * grid[:, 1] + 0.6 * grid[:, 0] ** 2,
        )
    )
    basis_fn = get_basis("quadratic_with_constant")
    res = fit_identified_model(grid, xdot, basis_fn=basis_fn, basis_name="quadratic_with_constant")
